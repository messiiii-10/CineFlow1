    ...
    namespace CineFlow.Controllers
    {
        public class HomeController : Controller
        {
            ...
            [HttpGet]
            public IActionResult SifreSifirla() => View(new ForgotPasswordViewModel());

            [HttpPost]
            [ValidateAntiForgeryToken]
            public async Task<IActionResult> SifreSifirla(ForgotPasswordViewModel model)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "Şifre sıfırlama için Firebase Authentication aktif olmalıdır.");
                    return View(model);
                }
                ...
                var resetResult = await _firebaseIdentityService.SendPasswordResetEmailAsync(model.Email.Trim());
                if (!resetResult.Succeeded)
                {
                    ModelState.AddModelError(string.Empty, resetResult.ErrorMessage ?? "Şifre sıfırlama e-postası gönderilemedi.");
                    return View(model);
                }

                TempData["AuthMessage"] = "Şifre sıfırlama bağlantısı e-posta adresine gönderildi.";
                return RedirectToAction("Login");
            }
